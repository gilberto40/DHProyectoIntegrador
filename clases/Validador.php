...
class Validador{
    ...
    static public function validarDatosAdministrador($usuario){
        $errores = [];
        // VALIDAR USER NAME
        $userName=$usuario->getUserName();
        $email =$usuario->getEmail();
    ...
    static public function validarEdit($post,$data){
        $errores =[];
        ...
        if(strlen($post["newNombre"]) == 0){
            ...
            $errores["password"] = "No puedes dejar este campo vacio";
        }else if(!password_verify($post['password'],$data[0]['password'])){
